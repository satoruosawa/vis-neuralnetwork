...
void setup() {
  ...
  NEURON_0_2 = new Neuron(new float[]{0.4, -0.1}, 0.3);
  NEURON_0_3 = new Neuron(new float[]{0.3, 0.5}, 0.1);
  NEURON_1_0 = new Neuron(new float[]{-0.2, 0.5, 0.3, -0.1}, -0.2);
...
void drawNeuron(float[] x, float size) {
  ...
  float p03 = NEURON_0_3.run(x);
  float p10 = NEURON_1_0.run(new float[]{p00, p01, p02, p03});
  if (p10 > 0.5) {
    fill(COLOR_TRUE);
  } else {
    fill(COLOR_FALSE);
...
void updateParameters() {
  ...
  NEURON_0_3.weight(0, CP5.getController("P03_WEIGHT0").getValue());
  NEURON_0_3.weight(1, CP5.getController("P03_WEIGHT1").getValue());
  NEURON_0_3.bias(CP5.getController("P03_BIAS").getValue());
  ...
  NEURON_1_0.weight(3, CP5.getController("P10_WEIGHT3").getValue());
